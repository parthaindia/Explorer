...
public class UpdateRouteService extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String routeMap = request.getParameter("routeMap");
            String routeId = request.getParameter("routeId");
            HashMap map = new Gson().fromJson(routeMap, new TypeToken<HashMap>() {
            }.getType());
            boolean output = new RouteManager().updateRoute(routeId, map);
            if (output) {
